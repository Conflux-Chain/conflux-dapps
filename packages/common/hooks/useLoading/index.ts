...
import { createPageLoading, createPageLoadingTransition } from './createPageLoading';
import { createSpinLoading, createSpinLoadingTransition } from './createSpinLoading';
import './index.css';

export const validBlur = { low: 1, middle: 1, high: 1, none: 1 } as const;
...
export let routerDOM: HTMLElement | null = null;
...
const useLoading = (
    {
        ...
        size,
        showBlur = 'low',
    }: { type: 'Page' | 'Spin'; targetDOM?: HTMLElement; delay?: number; size?: number; showBlur?: keyof typeof validBlur; } = {
        type: 'Page',
        delay: 0,
        showBlur: 'low',
    }
    ...
    useEffect(() => {
        loadingStatusRef.current = loading;
    }, [loading]);
    const delayTimerRef = useRef<number>();

    const clearTransitionRef = useRef<Function | null>(null);
    const loadingEleRef = useRef<HTMLDivElement | null>(null);
    ...
    const ref = useRef<any>(); // ref for return.

    useEffect(() => {
        let _targetDOM: HTMLElement | undefined | null = document.body;
        if (type !== 'Page') {
            _targetDOM = isDOMElement(targetDOM) ? targetDOM : isDOMElement(ref.current) ? ref.current : null;
        } else {
            _targetDOM = routerDOM || (routerDOM = document.body);
        }
        ...
        return () => {
            ...
            if (typeof delayTimerRef.current === 'number') clearTimeout(delayTimerRef.current);
            if (clearTransitionRef.current) clearTransitionRef.current(() => clearLoading(_targetDOM));
...
function isDOMElement(ele) {
    try {
        ...
        return typeof ele === 'object' && ele.nodeType === 1 && typeof ele.style === 'object' && typeof ele.ownerDocument === 'object';
    }
}
