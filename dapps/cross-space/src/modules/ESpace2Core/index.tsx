...
const Transfer2Bridge: React.FC<{ isShow: boolean; inTransfer: boolean; setInTransfer: React.Dispatch<React.SetStateAction<boolean>> }> = memo(
    ({ isShow, inTransfer, setInTransfer }) => {
        const i18n = useI18n(transitions);
        const isMetaMaskHostedByFluent = useIsMetaMaskHostedByFluent();
            ...
            <>
                <div className="mt-[24px] flex justify-between items-center h-[24px] text-[16px] text-[#3D3F4C] font-medium">
                    <span className="inline-flex items-center">
                        <span className="mr-[8px] px-[10px] h-[24px] leading-[24px] rounded-[4px] bg-[#F0F3FF] text-center text-[12px] text-[#808BE7]">
                            Step 1
                        </span>
                        Transfer Token
                    </span>
                    ...
                    {currentToken.isNative && (
                        <button
                            ...
                            type="button"
                        >
                            <span className="mr-[4px] text-[14px] text-[#808BE7] font-normal">{mode === 'normal' ? 'Advanced Mode' : 'Normal Mode'}</span>
...
const TransferNormalMode: React.FC<{ isShow: boolean; inTransfer: boolean; setInTransfer: React.Dispatch<React.SetStateAction<boolean>> }> = ({
    isShow,
    inTransfer,
    setInTransfer,
}) => {
    ...
    return (
        <form onSubmit={onSubmit}>
                    ...
                    suffix={
                        <button
                            className={cx(
                                ...
                                isBalanceGreaterThan0 && 'hover:underline'
                            )}
            ...
            <div className="text-[14px] leading-[18px] text-[#3D3F4C]">
                <span className="text-[#15C184]" id="eSpace-balance">
                    eSpace
                </span>{' '}
                Balance:
                {inTransfer && (
                    ...
                    <>
                        {currentTokenBalance ? (
                            ...
                            Unit.lessThan(currentTokenBalance, Unit.fromStandardUnit('0.000001', currentToken?.decimals)) ? (
                                ...
                                )} ${currentToken.evm_space_symbol}`}</span>
                            )
                        ) : (
                            <span id="eSpace2Core-currentTokenBalance" className="ml-[4px]">
                                loading...
            ...
            <div className={cx('mt-[8px] text-[14px] leading-[18px]', isCurrentTokenHasEnoughLiquidity ? 'text-[#3D3F4C]' : 'text-[#E96170]')}>
                ...
                {!isCurrentTokenHasEnoughLiquidity && (
                    <span>
                        {`Insufficient liquidity on Core, maximum liquidity is ${numFormat(
                            maximumLiquidity?.toDecimalStandardUnit(undefined, currentToken?.decimals)
                        )} ${currentToken.evm_space_symbol}`}
                    </span>
                )}
            </div>
        ...
        <>
            <div className="mt-[10px] pl-[16px] py-[12px] bg-[#F8F9FE]">
                ...
                <ul className="list-disc pl-[23px] text-[14px] text-[#898D9A] leading-[18px]">
                    <li>
                        Use <span className="text-[#15C184]">Conflux eSpace</span>.
                    </li>
                    <li>
                        Send your CFX to the <span className="text-[#3D3F4C] font-medium">following address</span>.
                    </li>
                    <li>
                        This address can <span className="text-[#3D3F4C] font-medium">only receive CFX.</span>
                    </li>
                </ul>
            </div>
