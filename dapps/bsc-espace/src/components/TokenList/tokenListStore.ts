...
import { getChainIndex, useCurrentFromChainIndex, type Token } from 'bsc-espace/src/store/index';
...
function getCurrentFromTokenList(currentFrom: 'crossChain' | 'eSpace' = 'eSpace') {
    const chainIndex = getChainIndex();
    let _tokens: Array<Token> = [];
    if (currentFrom === 'eSpace') {
        _tokens = Config.tokens;
    } else {
        _tokens = Config.chains[chainIndex].tokens;
    }
