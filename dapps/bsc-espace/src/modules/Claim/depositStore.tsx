...
export const depositListStore = create(
...
const mergeFetchedToLocal = (fetchedList: Array<Deposit>, account: string) => {
    ...
    const claimableList = localDepositList.filter(
        (localItem) =>
            !localItem.claim_tx_to && fetchedList.some((fetchedItem) => localItem.deposit_tx_hash === fetchedItem.deposit_tx_hash && !!fetchedItem.claim_tx_to)
    );
    const tempList = localDepositList.filter(
        (localItem) => !!localItem.deposit_tx_hash && !fetchedList.some((fetchedItem) => localItem.deposit_tx_hash === fetchedItem.deposit_tx_hash)
    );
...
export const startSubDepositList = () => {
    const unsub1 = walletStore.subscribe(
    ...
    const unsub2 = depositListStore.subscribe(
        (state) => state.tempDepositList,
        (tempDepositList) => {
            ...
            const checkReceipt = async () => {
                const checkReceiptRes: any = await Promise.allSettled(
                    tempDepositList.map((deposit) =>
                        provider!.request({
...
const mergeTempDepositToLocal = (
    checkReceiptRes: Array<{ status: 'fulfilled'; value: null | { transactionHash: string; status: string } }>,
    startNextTimer: () => void
) => {
    const preTempDepositList = depositListStore.getState().tempDepositList ?? [];
