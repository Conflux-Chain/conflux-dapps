...
const Cell: React.FC<Props> = ({ className, title, icon, TipContent, Content, children, ...props }) => {
    ...
    return (
        <div className={cx('relative px-[58px] pr-[12px] py-[14px] rounded-[8px] border-[1px] border-[#EAECEF] bg-white', className)} {...props}>
            ...
            <div className="flex items-center mb-[4px] text-[12px] h-[16px] leading-[16px] text-[#898D9A]">
                {title}
                {!!TipContent && (
                    <img
                        src={QuestionMark}
            ...
            <div className="text-[16px] h-[20px] leading-[20px] text-[#1B1B1C]">{chainId === Network.core.chainId && Content ? renderReactNode(Content) : '--'}</div>
        </div>
    );
};
