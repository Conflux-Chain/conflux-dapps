...
import { useCurrentVotingRoundEndTiming, fetchCurrentRound } from 'governance/src/store';
...
const Countdown: React.FC = () => {
    ...
    const currentVotingRoundEndTiming = useCurrentVotingRoundEndTiming();

    useLayoutEffect(() => {
        if (!currentVotingRoundEndTiming) return;
    ...
    }, [currentVotingRoundEndTiming]);

    return (
        <div className="flex justify-center gap-[16px]" ref={domRef}>
            {units.map(unit => <Unit key={unit} unit={unit} />)}
        </div>
    );
}

const Unit: React.FC<{ unit: typeof units[number]; }> = ({ unit }) => {
    return (
